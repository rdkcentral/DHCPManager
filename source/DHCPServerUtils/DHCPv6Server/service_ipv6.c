...
static int mbus_get(char *path, char *val, int size)
{
    ...
    if (CcspBaseIf_discComponentSupportingNamespace(bus_handle, CCSP_CR_COMPONENT_ID, path, CCSP_SUBSYS, &ppComponents, &compNum) != CCSP_SUCCESS) {
        DHCPMGR_LOG_INFO("failed to find component for %s \n", path);
        return -1;
    }
    ...
    if(CcspBaseIf_getParameterValues(bus_handle, ppDestComponentName, ppDestPath, paramNames, 1, &valNum, &parameterVal) != CCSP_SUCCESS) {
        DHCPMGR_LOG_INFO("failed to get value for %s \n", path);
...
static int get_dhcpv6s_pool_cfg(dhcpv6s_pool_cfg_t *cfg)
{
    int i = 0;
    ...
    if (p_opt == NULL) {
        DHCPMGR_LOG_INFO("calloc mem for pool options failed!\n");
        return -1;
    }
...
static int get_ia_info(__attribute__ ((unused)) char *config_file, ia_na_t *iana, ia_pd_t *iapd)
{
    char action[64] = {0};
    ...
    if (fd < 0) {
        DHCPMGR_LOG_INFO("open file %s failed!\n", config_file);
        return -1;
    }
    ...
    {
        DHCPMGR_LOG_INFO("read error: could not read message \n");
        close(fd);
        return -1;
    }
    if (!strncmp(config, "dibbler-client", strlen("dibbler-client")))
    {
        ...
        //DHCPMGR_LOG_INFO("%s -- %d !!! get configs from v6 client: %s \n", __FUNCTION__, __LINE__,p);
        ...
        if (sscanf(p, "%63s %63s %s %s %s %s %s %63s %d %s %s %s %s %s",
            ...
            DHCPMGR_LOG_INFO("Get the IA_NA and IA_PD failed.\n");
...
static int get_prefix_info(const char *prefix,  char *value, unsigned int val_len, unsigned int *prefix_len)
{
    ...
    if (i == 0) {
        DHCPMGR_LOG_INFO("[%s] error, there is not '/' in prefix:%s\n", __FUNCTION__, prefix);
        return -1;
    }
...
static int divide_ipv6_prefix()
{
    ...
    if (inet_pton(AF_INET6, mso_prefix.value, prefix) <= 0) {
        DHCPMGR_LOG_INFO("prefix inet_pton error!.\n");
        return -1;
    }
    ...
    if (enabled_iface_num > (1U << delta_bits)) {
    ...
    if (sub_prefixes == NULL) {
        DHCPMGR_LOG_INFO("calloc mem for sub-prefixes failed.\n");
        return -1;
    }
    ...
    for (i = 0; i < sub_prefix_num; i ++) {
        ...
        DHCPMGR_LOG_INFO("sub-prefix:%s/%d\n", p_prefix->value, p_prefix->len);
    ...
    if(sub_prefix_num == 0)
    {
        DHCPMGR_LOG_INFO("sub prefix num is zero.\n");
        return -1;
    }
    ...
    {
        DHCPMGR_LOG_INFO("iface prefix num for each sub-prefix failed.\n");
        return -1;
    }
...
int compute_global_ip(char *prefix, char *if_name, char *ipv6_addr, unsigned int addr_len)
{
    ...
    if (length > 64) {
        DHCPMGR_LOG_INFO("[%s] error, length is bigger than 64. prefix:%s, length:%d\n", __FUNCTION__, prefix, length);
        return -1;
    }
    ...
    //DHCPMGR_LOG_INFO("the first ipv6 addr part is:%s\n", globalIP);

    /* prepare second part */
    if (iface_get_hwaddr(if_name, mac, sizeof(mac)) != 0) {
        DHCPMGR_LOG_INFO("get the mac of %s error!\n", if_name);
        return -1;
    }
    ...
    DHCPMGR_LOG_INFO("the global ipv6 address is:%s\n", globalIP);
#ifdef MULTILAN_FEATURE
 ...
 */
static void report_no_lan_prefixes()
{
    unsigned int enabled_iface_num = 0;
    unsigned int l2_insts[MAX_LAN_IF_NUM] = {0};
    int i = 0;
...
static int lan_addr6_set()
{
    ...
    get_active_lanif(l2_insts, &enabled_iface_num);
    if (enabled_iface_num == 0) {
        DHCPMGR_LOG_INFO("no active lan interface.\n");
        return -1;
    }
...
#else
    get_active_lanif(l2_insts, &if_num);
    if (if_num == 0) {
        DHCPMGR_LOG_INFO("no active lan interface.\n");
        return -1;
    }
...
static int gen_dibbler_conf()
{
   ...
   /*Run scipt to config route */
    ...
    ret = get_ia_info(PROVISIONED_V6_CONFIG_FILE, &ia_na, &ia_pd);

    for (pool_index = 0; pool_index < dhcpv6s_cfg.pool_num; pool_index++) {
        dhcpv6s_pool_cfg.index = pool_index;
        if (get_dhcpv6s_pool_cfg(&dhcpv6s_pool_cfg) != 0)
            continue;
...
static int dhcpv6s_start()
{
    ...
    if (gen_dibbler_conf() != 0) {
        DHCPMGR_LOG_INFO("%s: fail to generate dibbler config\n", __FUNCTION__);
        return -1;
    }
#ifdef MULTILAN_FEATURE
    ...
    {
       DHCPMGR_LOG_INFO("%s: DHCPv6 Disabled. Dibbler start not required !\n", __FUNCTION__);
       return 0;
    }
#endif
    sleep(1);
...
int serv_ipv6_start(__attribute__((__unused__)) void *args)
{
    ...
    if (atoi(rtmod) != 1) { /* IPv6-only or Dual-Stack */
        ...
        if (!si6->wan_ready) {
            DHCPMGR_LOG_INFO("%s: IPv6-WAN is not ready !\n", __FUNCTION__);
            return -1;
        }
    } else {/* IPv4-only */
        return 0;
...
int serv_ipv6_stop(__attribute__((__unused__)) void *args)
{
    DHCPMGR_LOG_INFO("%s %d\n", __FUNCTION__, __LINE__);
    if (args != NULL)
        DHCPMGR_LOG_INFO("%s: args %s\n", __FUNCTION__, (char *)args);
...
int serv_ipv6_restart(__attribute__((__unused__)) void *args)
{
    if (serv_ipv6_stop("") != 0)
        DHCPMGR_LOG_INFO("%s: serv_ipv6_stop error\n", __FUNCTION__);

    return serv_ipv6_start("");
}
...
int serv_ipv6_init()
{
    ...
    si6 = malloc(sizeof(struct serv_ipv6));
...
int serv_ipv6_update(void)
{
    ...
    switch(atoi(buf)) {
        ...
        default:
#ifdef MULTILAN_FEATURE
            DHCPMGR_LOG_INFO("%s: unknown erouter topology mode, settinf default mode to FAVOR_WIDTH \n", __FUNCTION__);
            si6->tpmod = FAVOR_WIDTH;
#else
            DHCPMGR_LOG_INFO("%s: unknown erouter topology mode.\n", __FUNCTION__);
