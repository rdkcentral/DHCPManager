...
int DhcpMgr_Dhcp_Recovery_Start()
{
    ...
    int ret_val = DHCPMgr_loadDhcpLeases(DHCP_VERSION_4);
...
int DHCPMgr_storeDhcpLease(char* ifname, void* newLease, int dhcpVersion,ULONG instanceNum)
{
    ...
    // Need to add the v6 data also PCOSA_DML_DHCPCV6_FULL pDhcpc6 = NULL;
    ...
    {
        PCOSA_DML_DHCPC_FULL temp = NULL;
        temp = (PCOSA_DML_DHCPC_FULL)newLease;
        pDhcpc = (PCOSA_DML_DHCPC_FULL)newLease;
        ...
        if (temp->currentLease != NULL)
        {
//            DHCPMGR_LOG_INFO("%s:%d <<DEBUG>> newLease->currentLease is VALID\n", __FUNCTION__, __LINE__);
            memcpy(pDhcpc->currentLease,temp->currentLease, sizeof(DHCPv4_PLUGIN_MSG));
            pDhcpc->currentLease->next = NULL;
        }
        else
        {
            DHCPMGR_LOG_ERROR("%s:%d  newLease->currentLease is NULL\n", __FUNCTION__, __LINE__);
    ...
    {
 //       pDhcpc6 = (PCOSA_DML_DHCPCV6_FULL)newLease;
    }
...
//    DHCPMGR_LOG_INFO("%s:%d <<DEBUG>> filePath=%s\n", __FUNCTION__, __LINE__, filePath);
    ...
    if (fwrite(pDhcpc, sizeof(COSA_DML_DHCPC_FULL), 1, file) != 1) {
