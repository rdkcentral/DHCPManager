    ...
    )
{
    ...
    if (vsystem("/bin/sh /etc/utopia/service.d/set_hostname.sh") != 0) {
        DHCPMGR_LOG_INFO("fail to set resolv.conf");
        return ANSC_STATUS_FAILURE;
    }
    ...
    )
{
    ...
    if (vsystem("/bin/sh /etc/utopia/service.d/set_wandomain.sh") != 0) {
        DHCPMGR_LOG_INFO("fail to set wan domain name");
        return ANSC_STATUS_FAILURE;
    }
...
{
    CCSP_MESSAGE_BUS_INFO *bus_info = (CCSP_MESSAGE_BUS_INFO *)bus_handle;
    DHCPMGR_LOG_INFO("WiFi is going to reboot");
    pthread_detach(pthread_self());
...
CosaDmlDcResetBr0(char *ip, char *sub) {
    ...
    DHCPMGR_LOG_INFO("-- %s %d %s", __func__, __LINE__, objValue);
    ...
    ret = CcspBaseIf_discComponentSupportingNamespace(
        ...
        &size);
    if ( ret != CCSP_SUCCESS ) {
        DHCPMGR_LOG_ERROR("Error:'%s' is not exist", objName);
        return ANSC_STATUS_FAILURE;
    }
    ...
    if (ret != CCSP_SUCCESS && faultParam) {
        DHCPMGR_LOG_ERROR("Error:Failed to SetValue for param '%s'", faultParam);
        bus_info->freefunc(faultParam);
    }
...
//TCCBR-4087
        ...
        if(delay) {
            if(delay_time)
            {
                DHCPMGR_LOG_INFO("Device is going to reboot in %d seconds", delay_time);
            ...
            {
                DHCPMGR_LOG_INFO("Device is going to reboot in 5 seconds");
                CcspTraceWarning(("RebootDevice:Device is going to reboot after taking log backups \n"));
                //system("(sleep 5 && reboot) &");
                CosaDmlDcSaveWiFiHealthStatusintoNVRAM( );
                sleep(5);
            ...
            DHCPMGR_LOG_INFO("Device is going to reboot now");
            CcspTraceWarning(("RebootDevice:Device is going to reboot after taking log backups \n"));
            //system("reboot");
            CosaDmlDcSaveWiFiHealthStatusintoNVRAM( );
            v_secure_system("/rdklogger/backupLogs.sh &");
    ...
    if (router) {
        DHCPMGR_LOG_INFO("Router is going to reboot");
    ...
    if (voip) {
        DHCPMGR_LOG_INFO("VoIP is going to reboot");
        // TODO:
    }
    if (dect) {
        DHCPMGR_LOG_INFO("Dect is going to reboot");
        // TODO:
    }
    if (moca) {
        DHCPMGR_LOG_INFO("MoCA is going to reboot");
        // TODO:
    }
...
#define ID "/tmp/elxrretyt-lxy.swr"
    ...
    v_secure_system("rm -f /nvram/moca.conf.default");
    v_secure_system("rm -f /nvram/*.moca0");
#endif
    v_secure_system("rm -f /nvram/mesh_enabled");
#if defined (_ARRIS_XB6_PRODUCT_REQ_)
    v_secure_system("rm -f /nvram/etc/passwd");
    v_secure_system( "arris_rpc_client arm nvm_reset" );
    ...
    )
{
    ...
    if (pValue == NULL || pValue[0] == '\0')
        ...
        if(delay) {
            if(delay_time)
            {
                DHCPMGR_LOG_INFO("CosaDmlDcSetFactoryReset: is going to wait for %d seconds", delay_time);
                CcspTraceWarning(("CosaDmlDcSetFactoryReset: is going to wait for %d seconds\n", delay_time));
                sleep (delay_time);
            }
            else
            {
                DHCPMGR_LOG_INFO("CosaDmlDcSetFactoryReset: is going to wait for 5 seconds");
...
#define ROUTER_RESET_COUNT_FILE "/nvram/.router_reset_count"
    ...
    if (factory_reset_mask & FR_WIFI) {
        ...
        //Remove WiFi Motion State
        v_secure_system("rm -f /nvram/lb_stats");
#endif
    }
    ...
    )
{
    ...
    DHCPMGR_LOG_INFO("igmpproxy is %s", (*pFlag)==TRUE?"alive":"dead");
    return ANSC_STATUS_SUCCESS;
}
    ...
    )
{
    ...
    DHCPMGR_LOG_INFO("dnsproxy is %s", (*pFlag)==TRUE?"alive":"dead");
    return ANSC_STATUS_SUCCESS;
}
...
static void configBridgeMode(int bEnable) {
    ...
    char brpdm[50];
    char brmode[5];
    PCOSA_NOTIFY_WIFI pnotifypara = (PCOSA_NOTIFY_WIFI)AnscAllocateMemory(sizeof(COSA_NOTIFY_WIFI)); /* free unused resource before return */
...
//         varstruct.type = ccsp_boolean;
    ...
    g_SetParamValueBool(brpdm, bEnable);
    /*  Trigger only router mode from here,
    ...
    if (ppComponents == NULL && initWifiComp()) {
        ...
        AnscFreeMemory( pnotifypara ); /* free unused resource before return */
        return;
    }
    totalticket += 1;
