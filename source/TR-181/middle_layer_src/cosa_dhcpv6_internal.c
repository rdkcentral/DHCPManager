    ...
    )
{
    ...
    CosaDmlDhcpv6SMsgHandler((ANSC_HANDLE)pMyObject);
    ...
    )
{
    ...
    for ( ulIndex = 0; ulIndex < ulEntryCount; ulIndex++ )
    {
        ...
        for ( ulIndex2 = 0; ulIndex2 < ulEntryCount2; ulIndex2++ )
        {
            ...
            {
                returnStatus = ANSC_STATUS_FAILURE;
                AnscFreeMemory(pDhcpv6SntOpt); /* free unused resource before exit*/
                pDhcpv6SntOpt = NULL;
                goto EXIT3;
            }
...
EXIT1:
    ...
    /* Logically dead code - EXIT1 invoke only when ptr is NULL*/ 
