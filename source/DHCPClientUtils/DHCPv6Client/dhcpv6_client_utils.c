    ...
    if (syscfg_get(NULL, "dslite_enable", dslite_enable, sizeof(dslite_enable)) == 0)
    {
        if (strcmp(dslite_enable, "true") == 0)
        {
            ...
            if (strcmp(dslite_enable, "true") == 0)
            {
                ...
                {
                    DBG_PRINT("<<DEBUG>>%s %d: failed to add REQUEST option OPTION_AFTR_NAME to list\n", __FUNCTION__, __LINE__);
                    return FAILURE;
                }
            }
        }
...
pid_t start_dhcpv6_client (dhcp_params * params)
{
    ...
    if (pid > 0)
    {
        DBG_PRINT("<<DEBUG>>%s %d: another instance of %s running on %s \n", __FUNCTION__, __LINE__, DIBBLER_CLIENT, params->ifname);
        return FAILURE;
    }
    ...
    {
        DBG_PRINT("<<DEBUG>>%s %d: Fail to open sysevent.\n", __FUNCTION__, __LINE__);
    ...
    while (waitTime > 0)
    {
        ...
        if(fp_dad != NULL) 
        {
            ...
            DBG_PRINT("<<DEBUG>>%s %d: interface still tentative: %s\n", __FUNCTION__, __LINE__, buffer);
            pclose(fp_dad);
        }
    ...
    if (waitTime <= 0)
    {
        DBG_PRINT("<<DEBUG>>%s %d: interface %s doesnt have link local address\n", __FUNCTION__, __LINE__, params->ifname);
    ...
    DBG_PRINT("<<DEBUG>>%s %d: Starting Dibbler Clients\n", __FUNCTION__, __LINE__);
