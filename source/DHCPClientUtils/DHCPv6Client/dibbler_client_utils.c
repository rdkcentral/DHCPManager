...
static int copy_file (char * src, char * dst)
{
    ...
    if (fout == NULL)
    {
        DBG_PRINT("<<DEBUG>>%s %d: failed to open file %s\n", __FUNCTION__, __LINE__, dst);
        return FAILURE;
    }
    ...
    DBG_PRINT("<<DEBUG>>%s %d: successfully copied content from %s to %s\n", __FUNCTION__, __LINE__, src, dst);
    return SUCCESS;
...
static int dibbler_client_prepare_config (dibbler_client_info * client_info)
{
    ...
    if (fout == NULL)
    {
        DBG_PRINT("<<DEBUG>>%s %d: unable to open tmp file: %s\n", __FUNCTION__, __LINE__, DIBBLER_TMP_CONFIG_FILE);
        return FAILURE;
    }
    ...
    {
        DBG_PRINT("<<DEBUG>>%s %d: created directory %s\n", __FUNCTION__, __LINE__, client_info->config_path);
    }
    else
    {
        DBG_PRINT("<<DEBUG>>%s %d: Directory already exists / not created  %s\n", __FUNCTION__, __LINE__, client_info->config_path);
    }
    ...
    if (access(DIBBLER_DEFAULT_CONFIG_FILE, F_OK) == 0) {
        DBG_PRINT("<<DEBUG>>%s %d: link already exists, continuing\n", __FUNCTION__, __LINE__);
        return SUCCESS;
    } else {
        // Creating the link only if it doesn't already exist
        if (link(DIBBLER_TMP_CONFIG_FILE, DIBBLER_DEFAULT_CONFIG_FILE) == 0) {
            DBG_PRINT("<<DEBUG>>%s %d: link created successfully\n", __FUNCTION__, __LINE__);
            return SUCCESS;
        } else {
            DBG_PRINT("<<DEBUG>>%s %d: unable to create link: %s\n", __FUNCTION__, __LINE__, strerror(errno));
            return FAILURE;
        }
    }
...
pid_t start_dibbler (dhcp_params * params, dhcp_opt_list * req_opt_list, dhcp_opt_list * send_opt_list)
{
    ...
    {
        DBG_PRINT("<<DEBUG>>%s %d: Unable to get DHCPv6 REQ OPT.\n", __FUNCTION__, __LINE__);
        return FAILURE;
    }

    DBG_PRINT("<<DEBUG>>%s %d: Starting dibbler with config %s\n", __FUNCTION__, __LINE__, client_info.config_path);
    ...
    if (ret <= 0)
    {
        DBG_PRINT("<<DEBUG>>%s %d: unable to start dibbler-client %d.\n", __FUNCTION__, __LINE__, ret);
        return FAILURE;
    }
...
int stop_dibbler (dhcp_params * params)
{
    ...
    while (waitTime > 0)
    {
        ...
        if (stat(cmd, &sts) == -1 && errno == ENOENT) {
            ...
            DBG_PRINT("<<DEBUG>>%s %d: dibbler-client exited\n", __FUNCTION__, __LINE__);
            break;
        }
    ...
    if (waitTime <= 0)
    {
        DBG_PRINT("<<DEBUG>>%s %d: Waited for %d millisec, dibbler-client still running\n", __FUNCTION__, __LINE__, DIBBLER_CLIENT_TERMINATE_TIMEOUT);
        return FAILURE;
    }
